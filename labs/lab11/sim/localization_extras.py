...
MAX_VALID_RANGE = 3.0  # meters

# Cap residuals fed to the sensor likelihood. Single-ray outliers (rays
# grazing past a corner the discretized cell does not see, ToF dropouts,
# unmapped objects) otherwise crush a cell's likelihood end-to-end.
RESIDUAL_CLIP = 0.5  # meters
...
class Localization(BaseLocalization):
    ...
    def compute_control(self, cur_pose, prev_pose):
        ...
        """
        delta_rot_1 = self.mapper.normalize_angle(
            math.degrees(
                np.arctan2(cur_pose[1] - prev_pose[1], cur_pose[0] - prev_pose[0])
            )
            - prev_pose[2]
        )
    ...
    def odom_motion_model(self, cur_pose, prev_pose, u):
        ...
        """
        exp_delta_rot_1, exp_delta_trans, exp_delta_rot_2 = self.compute_control(
            cur_pose, prev_pose
        )

        prob = (
            self.gaussian(
                self.mapper.normalize_angle(exp_delta_rot_1 - u[0]),
                0,
                self.odom_rot_sigma,
            )
            * self.gaussian(exp_delta_trans, u[1], self.odom_trans_sigma)
            * self.gaussian(
                self.mapper.normalize_angle(exp_delta_rot_2 - u[2]),
                0,
                self.odom_rot_sigma,
            )
        )
                        ...
                        for cur_x in range(0, self.mapper.MAX_CELLS_X):
                            for cur_y in range(0, self.mapper.MAX_CELLS_Y):
                                for cur_a in range(0, self.mapper.MAX_CELLS_A):
                                    self.bel_bar[cur_x, cur_y, cur_a] = self.bel_bar[
                                        cur_x, cur_y, cur_a
                                    ] + (
                                        self.odom_motion_model(
                                            self.mapper.from_map(cur_x, cur_y, cur_a),
                                            self.mapper.from_map(
                                                prev_x, prev_y, prev_a
                                            ),
                                            u,
                                        )
                                        * self.bel[prev_x, prev_y, prev_a]
                                    )

        self.bel_bar = self.bel_bar / np.sum(self.bel_bar)
        LOG.info(" | Prediction Time: {:.3f} secs".format(time.time() - start_time))
    ...
    def update_step(self):
        ...
        for i in range(0, self.mapper.OBS_PER_CELL):
            z = self.obs_range_data[i][0]
            if z > MAX_VALID_RANGE:
                continue
            # Per-cell, per-bucket sigma: ToF noise floor combined in
            # quadrature with the spread of oversampled rays feeding that
            # bucket. Buckets that straddle a discontinuity get a large
            # std automatically and stop dominating the likelihood.
            sigma = np.sqrt(
                self.sensor_sigma**2 + self.mapper.obs_view_stds[:, :, :, i] ** 2
            )
            r = np.clip(
                self.mapper.obs_views[:, :, :, i] - z,
                -RESIDUAL_CLIP,
                RESIDUAL_CLIP,
            )
            self.bel = self.bel * self.gaussian(r, 0, sigma)

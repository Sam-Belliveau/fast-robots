...
class ArtemisBLEController(BaseBLEController):
    ...
    def __init__(self, config="connection.yaml", max_write_length=150):
        if ArtemisBLEController._instantiated:
            raise Exception(
        ...
        super(ArtemisBLEController, self).__init__(
            address, service_uuid, max_write_length
        )
    ...
    def send_command(self, cmd_type, data):
        ...
        if len(cmd_string) < self.max_write_length:
            ...
            raise Exception(
                "Cannot write string larger than {} bytes".format(self.max_write_length)
            )
